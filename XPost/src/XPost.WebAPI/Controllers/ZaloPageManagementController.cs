...
public class ZaloPageManagementController : ControllerBase
{
    ...
    /// <summary>
    /// DELETE /api/zalo-pages/{accountId}/conversations/{convId}
    /// Deletes a conversation and its messages from local DB.
    /// </summary>
    ...
    public async Task<IActionResult> DeleteConversation(Guid accountId, Guid convId)
    {
        try
        {
            ...
            var conv = await _dbContext.ZaloConversations.FirstOrDefaultAsync(c => c.Id == convId && c.SocialAccountId == accountId);
            if (conv == null) return NotFound(new { error = "Conversation not found." });

            // Delete messages in conversation
            var messages = _dbContext.ZaloMessages.Where(m => m.ConversationId == convId);
            _dbContext.ZaloMessages.RemoveRange(messages);

            // Delete conversation
            _dbContext.ZaloConversations.Remove(conv);
        ...
        {
            _logger.LogError(ex, "Error deleting Zalo conversation {ConvId}", convId);
        ...
        CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Error getting Zalo conversation orders for {Psid}", psid);
        ...
        CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Error getting Zalo page orders");
        ...
        CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Error deleting Zalo order {OrderId}", orderId);
