...
# Note: Using escaped backticks ` for JS template literals to prevent PS interpolation
$replacement = @"
            {/* Bulk Action Bar */}
            {selectedIds.size > 0 && (
...
if ($content.Contains($target)) {
    ...
    Write-Output "Successfully updated Keywords.tsx"
} else {
    Write-Error "Target content not found in Keywords.tsx"
}
